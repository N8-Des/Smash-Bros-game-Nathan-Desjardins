...
public class OptionsSelecter : MonoBehaviour {
    ...
    public OptBase currentlySelected;
    public List<OptBase> Options = new List<OptBase>();
    ...
    public void testForMovement()
    {
        if (canSelect)
        {
            currentlySelected = Options[onNumber];
            lastInput = getLastInput();
            canSelect = false;
            if (lastInput == "Down" && !player1Selected)
            {
                if (onNumber + 1 >= Options.Count)
                {
            ...
            else if (lastInput == "Up" && !player1Selected)
            {
                if (onNumber - 1 < 0)
                {
                    currentlySelected.isSelected = false;
                    onNumber = Options.Count - 1;
                    currentlySelected = Options[onNumber];
                    currentlySelected.isSelected = true;
                    Invoke("waitToSelect", 0.3f);
                }
